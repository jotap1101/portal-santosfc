...
<head>
    ...
    <title>Painel do administrador - Vídeos</title>
...
<body>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 pt-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h1">Gerenciar Vídeos</h1>
                </div>
                <!-- Botão -->
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#adicionarVideo">Adicionar Vídeo</button>

                <!-- Tabela -->
                <div class="table-responsive mt-3">
                    <table class="table table-striped table-sm" id="myTable">
                        <caption>Vídeos cadastrados</caption>
                        <thead>
                            <tr>
                                <th scope="col">#</th>
                        ...
                        <tbody>
                            ...
                            while ($fetch = mysqli_fetch_assoc($result)) {
                                $data_envio = new DateTime($fetch['data_upload']);
                            ?>
                                <tr>
                                    <td><?= $fetch['id'] ?></td>
                                    <td><?= $fetch['link'] ?></td>
                                    <td><?= ($fetch['data_upload'] == NULL) ? "-" : $data_envio->format('d/m/Y') . " às " . $data_envio->format('H:i:s') ?></td>
                                    <td>
                                        <button type="button" value="<?= $fetch['id']; ?>" class="btnVisualizarVideo btn btn-info btn-sm">Visualizar</button>
                                        <a href="admin-editar.php?page=video&id=<?= $fetch['id']; ?>" type="button" value="<?= $fetch['id']; ?>" class="btnEditarVideo btn btn-success btn-sm">Editar</a>
                                        <button type="button" value="<?= $fetch['id']; ?>" class="btnExcluirVideo btn btn-danger btn-sm">Excluir</button>
                                    </td>
                                </tr>
                            <?php
                            }
                ...
                <!-- modal adicionar -->
                <div class="modal fade" id="adicionarVideo" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="exampleModalLabel">Novo Video</h1>
                ...
                <!-- modal view -->
                <div class="modal fade" id="visualizarVideo" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="exampleModalLabel">Visualizar Video</h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <div class="mb-3">
    ...
    <script>
        $(document).on('submit', '#adicionar', function(e) {
            ...
            formData.append("adicionar_video", true);
        ...
        $(document).on('click', '.btnVisualizarVideo', function() {

            var idVideo = $(this).val();
            $.ajax({
                type: "GET",
                url: "code.php?idVideo=" + idVideo,
                success: function(response) {
                    var res = jQuery.parseJSON(response);
                    if (res.status == 404) {
                        alert(res.message);
                    } else if (res.status == 200) {
                        $('#view_link').text(res.data.link);

                        $('#visualizarVideo').modal('show');
                    }
                }
            });
        });

        $(document).on('click', '.btnExcluirVideo', function(e) {
            e.preventDefault();
